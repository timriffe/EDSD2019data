...
NAT %>% 
  mutate(time = hm(paste(hour, minute,sep=":"))) %>% 
  filter(mage >= 17,
         mage <= 35) %>% 
  ggplot(mapping = aes(x = dfrac, y = mage, fill = logit(bfrac))) +
  geom_tile() + 
  xlim(0,1) + 
